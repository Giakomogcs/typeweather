...
export async function getWeatherByCity({ latitude, longitude }: GetWeatherByCity): Promise<GetWeatherByCityResponseProps> {
  const { data } = await api.get<WeatherAPIResponse>(`/forecast?lat=${latitude}&lon=${longitude}`);
